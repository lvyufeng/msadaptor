...
class Backend:
    ...
    def broadcast(self, tensors: List[torch.Tensor], opts: Optional[Any] = None):
        raise NotImplementedError(f"Backend {self.get_backend_name()} does not support broadcast.")

    def allreduce(self, tensors: List[torch.Tensor], opts: Optional[Any] = None):
        raise NotImplementedError(f"Backend {self.get_backend_name()} does not support allreduce.")

    def allreduce_sparse(self, tensors: List[torch.Tensor], opts: Optional[Any] = None):
        raise NotImplementedError(f"Backend {self.get_backend_name()} does not support allreduce sparse.")

    def allreduce_coalesced(self, tensors: List[torch.Tensor], opts: Optional[Any] = None):
        raise NotImplementedError(f"Backend {self.get_backend_name()} does not support allreduce_coalesced.")

    def reduce(self, tensors: List[torch.Tensor], opts: Optional[Any] = None):
        raise NotImplementedError(f"Backend {self.get_backend_name()} does not support reduce.")

    def allgather(self, output_tensors: List[List[torch.Tensor]], input_tensors: List[torch.Tensor], opts: Optional[Any] = None):
        raise NotImplementedError(f"Backend {self.get_backend_name()} does not support allgather.")

    def _allgather_base(self, output_buffer: torch.Tensor, input_buffer: torch.Tensor, opts: Optional[Any] = None):
        raise NotImplementedError(f"Backend {self.get_backend_name()} does not support _allgather_base.")

    def allgather_coalesced(self, output_tensor_lists: List[List[torch.Tensor]], input_tensors: List[torch.Tensor], opts: Optional[Any] = None):
        raise NotImplementedError(f"Backend {self.get_backend_name()} does not support allgather_coalesced.")

    def allgather_into_tensor_coalesced(self, outputs: List[torch.Tensor], inputs: List[torch.Tensor], opts: Optional[Any] = None):
        raise NotImplementedError(f"Backend {self.get_backend_name()} does not support allgather_into_tensor_coalesced.")

    def gather(self, output_tensors: List[List[torch.Tensor]], input_tensors: List[torch.Tensor], opts: Optional[Any] = None):
        raise NotImplementedError(f"Backend {self.get_backend_name()} does not support gather.")

    def scatter(self, output_tensors: List[torch.Tensor], input_tensors: List[List[torch.Tensor]], opts: Optional[Any] = None):
        raise NotImplementedError(f"Backend {self.get_backend_name()} does not support scatter.")

    def reduce_scatter(self, output_tensors: List[torch.Tensor], input_tensors: List[List[torch.Tensor]], opts: Optional[Any] = None):
        raise NotImplementedError(f"Backend {self.get_backend_name()} does not support reduce_scatter.")

    def _reduce_scatter_base(self, output_buffer: torch.Tensor, input_buffer: torch.Tensor, opts: Optional[Any] = None):
        raise NotImplementedError(f"Backend {self.get_backend_name()} does not support _reduce_scatter_base.")

    def reduce_scatter_tensor_coalesced(self, outputs: List[torch.Tensor], inputs: List[torch.Tensor], opts: Optional[Any] = None):
        raise NotImplementedError(f"Backend {self.get_backend_name()} does not support reduce_scatter_tensor_coalesced.")

    def alltoall_base(self, output_buffer: torch.Tensor, input_buffer: torch.Tensor, output_split_sizes: List[int], input_split_sizes: List[int], opts: Optional[Any] = None):
        raise NotImplementedError(f"Backend {self.get_backend_name()} does not support alltoall_base.")

    def alltoall(self, output_tensors: List[torch.Tensor], input_tensors: List[torch.Tensor], opts: Optional[Any] = None):
    ...
    def send(self, tensors: List[torch.Tensor], dst_rank: int, tag: int):
        raise NotImplementedError(f"Backend {self.get_backend_name()} does not support send.")

    def recv(self, tensors: List[torch.Tensor], src_rank: int, tag: int):
        raise NotImplementedError(f"Backend {self.get_backend_name()} does not support recv.")

    def recv_anysource(self, tensors: List[torch.Tensor], tag: int):
    ...
    def get_bound_device_id(self) -> Optional[torch.device]:
        return self.bound_device_id_

    def eager_connect_single_device(self, device: torch.device):
        pass

    def set_bound_device_id(self, device: Optional[torch.device]):
        if device:
