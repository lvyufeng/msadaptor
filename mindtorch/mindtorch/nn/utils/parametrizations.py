...
from .weight_norm import _weight_norm, norm_except_dim
...
def _is_orthogonal(Q, eps=None):
    ...
    Id = ops.eye(k, dtype=Q.dtype)
    # A reasonable eps, but not too large
    eps = 10.0 * n * float(ops.finfo(Q.dtype).eps)
    return ops.allclose(Q.mH @ Q, Id, atol=eps)
...
def _make_orthogonal(A):
    ...
    """
    X, tau = ops.geqrf(A)
    Q = ops.linalg.householder_product(X, tau)
...
class _Orthogonal(Module):
    ...
    def forward(self, X: mindspore.Tensor) -> mindspore.Tensor:
        ...
        if self.orthogonal_map in (_OrthMaps.matrix_exp, _OrthMaps.cayley):
            ...
            if self.orthogonal_map == _OrthMaps.matrix_exp:
                ...
                Id = ops.eye(n, dtype=A.dtype)
                Q = ops.linalg.solve(
                    ops.add(Id, A, alpha=-0.5), ops.add(Id, A, alpha=0.5)
                )
            ...
            Q = ops.linalg.householder_product(A, tau)
    ...
    def right_inverse(self, Q: mindspore.Tensor) -> mindspore.Tensor:
        ...
        if not hasattr(self, "base"):
            ...
            if self.orthogonal_map in (_OrthMaps.cayley, _OrthMaps.matrix_exp):
            ...
            neg_Id = ops.zeros_like(Q_init)
            neg_Id = neg_Id.diagonal(dim1=-2, dim2=-1).fill(-1.0)
            return neg_Id
...
class _SpectralNorm(Module):
    ...
    def _power_method(self, weight_mat: mindspore.Tensor, n_power_iterations: int) -> None:
        ...
        for _ in range(n_power_iterations):
            ...
            self._u = F.normalize(
                ops.mv(weight_mat, self._v),  # pylint: disable=access-member-before-definition
                dim=0,
                eps=self.eps,
            )
            self._v = F.normalize(
                ops.mv(weight_mat.H, self._u),  # type: ignore[has-type]
                dim=0,
                eps=self.eps,
            )
