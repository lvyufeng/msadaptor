...
def float_power(input, exponent):
    if isinstance(input, torch.Tensor) and isinstance(exponent, numbers.Number):
        return execute("pow_tensor_scalar", input, exponent)
    if isinstance(input, numbers.Number) and isinstance(exponent, torch.Tensor):
...
def fmod(input, other):
    if isinstance(input, torch.Tensor) and isinstance(other, numbers.Number):
        return execute("fmod_scalar", input, other)
    if isinstance(input, numbers.Number) and isinstance(other, torch.Tensor):
        return execute("fmod_scalar", input, other)
...
def xlogy(input, other):
    ...
    if isinstance(input, torch.Tensor) and isinstance(other, (float, int, bool)):
        return execute("xlogy_scalar_other", input, other)
    if isinstance(input, (float, int, bool)) and isinstance(other, torch.Tensor):
